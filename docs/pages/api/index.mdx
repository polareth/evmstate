...
```typescript twoslash
import type { TraceStateOptions, LabeledState } from "@polareth/evmstate";
import type { Address } from "tevm";

// @ts-expect-error - Function implementation is missing
function traceState(options: TraceStateOptions): Promise<Record<Address, LabeledState>>;
```
...
class Tracer {
  ...
  // @ts-expect-error - Function implementation is missing
  traceState(txOptions: TraceStateTxParams): Promise<Record<Address, LabeledState>>;
}
```
